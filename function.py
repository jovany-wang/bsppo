...
#定义waitpacketsize的大小
def wait_packetSize(time,arrival_data_matrix0,arrival_data_matrix1,arrival_data_matrix2,waitpacketSize0,wait_packetSize1,wait_packetSize2):
...
def BS_power(time, action,BSmodel0,BSmodel1,BSmodel2, trans_rate0,transiotion_time_ratio0,trans_rate1,transiotion_time_ratio1,trans_rate2,transiotion_time_ratio2,wait_packetSize0, wait_packetSize1, wait_packetSize2,arrival_data_matrix0,arrival_data_matrix1,arrival_data_matrix2,energy_sum,transition_time0,transition_time1,transition_time2):
    ...
    if transition_time0 != 0:#说明当前处于work model转换阶段
        ...
        if BSmodel0 == 0: #activate  #这里不能根据state中的BSmodel来判断，要根据action中的model来判断，因为这是根据action转换到下一个stata的过程
            available_trans0 = trans_rate0 * (1 - transiotion_time_ratio0)
            if wait_packetSize0 >= available_trans0: #说明使用当前全部的1ms才能刚好处理或处理不完data
                #先判断zooming的大小
                if action[3] == 1: #当前处于red圆
                    #计算energy
                    power_coms0 = Vars.power_trans_full_red * (1-transiotion_time_ratio0)
                elif action[3] == 2: #当前处于black圆
                    power_coms0 = Vars.power_trans_full_black * (1 - transiotion_time_ratio0)
                elif action[3] == 3: #当前处于green圆
                    power_coms0 = Vars.power_trans_full_green  * (1 - transiotion_time_ratio0)
                ...
                if action[3] == 1:
                    power_coms0 = (Vars.power_trans_full_red* (wait_packetSize0/available_trans0) + Vars.power_idle_red * (1-wait_packetSize0/trans_rate0))*(1-transiotion_time_ratio0)/1000  #两部分功率：传输过程中 + 空闲状态
                elif action[3] == 2:
                    power_coms0 = (Vars.power_trans_full_black * (wait_packetSize0 / available_trans0) + Vars.power_idle_black * (1 - wait_packetSize0 / trans_rate0)) * (1 - transiotion_time_ratio0) / 1000
                elif action[3] == 3:
                    power_coms0 = (Vars.power_trans_full_green * (wait_packetSize0 / available_trans0) + Vars.power_idle_green * (1 - wait_packetSize0 / trans_rate0)) * (1 - transiotion_time_ratio0) / 1000
                ...
                if action[3] == 1:
                    power_coms0 = Vars.power_idle_red * (1-transiotion_time_ratio0) #没有等待的包，则传输功率为空闲状态减去转换的时间
                elif action[3] == 2:
                    power_coms0 = Vars.power_idle_black * (1 - transiotion_time_ratio0)
                elif action[3] == 3:
                    power_coms0 = Vars.power_idle_green * (1 - transiotion_time_ratio0)
                energy_sum += power_coms0
        elif BSmodel0 == 1: #BS0处于SM1状态
            power_coms0 = Vars.power_SM1 * (1-transiotion_time_ratio0)  #除去转换的时间，剩余的就是睡眠时间了
            energy_sum += power_coms0  #将能耗加到总能耗中
        elif BSmodel0 == 2:  #SM2
            power_coms0 = Vars.power_SM2 * (1-transiotion_time_ratio0)
            energy_sum += power_coms0
        elif BSmodel0 == 3:  #SM3
            power_coms0 = Vars.power_SM2 * (1-transiotion_time_ratio0)
            energy_sum += power_coms0
        transiotion_time_ratio0 = 0
    ...
    if transition_time1 != 0:#说明当前处于work model转换阶段
        ...
        if BSmodel1 == 0:
            available_trans1 = trans_rate1 * (1 - transiotion_time_ratio1)
            if wait_packetSize1 >= available_trans1:
                if action[4] == 1: #当前处于red圆
                    #计算energy
                    power_coms1 = Vars.power_trans_full_red * (1-transiotion_time_ratio1)
                elif action[4] == 2: #当前处于black圆
                    power_coms1 = Vars.power_trans_full_black * (1 - transiotion_time_ratio1)
                elif action[4] == 3: #当前处于green圆
                    power_coms1 = Vars.power_trans_full_green * (1 - transiotion_time_ratio1)
                # 计算energy
                ...
                if action[4] == 1:
                    power_coms1 = (Vars.power_trans_full_red * (wait_packetSize1 / available_trans1) + Vars.power_idle_red * (1 - wait_packetSize1 / trans_rate1)) * (1 - transiotion_time_ratio1) / 1000
                elif action[4] == 2:
                    power_coms1 = (Vars.power_trans_full_black * (wait_packetSize1 / available_trans1) + Vars.power_idle_black * (1 - wait_packetSize1 / trans_rate1)) * (1 - transiotion_time_ratio1) / 1000
                elif action[4] == 3:
                    power_coms1 = (Vars.power_trans_full_green * (wait_packetSize1 / available_trans1) + Vars.power_idle_green * (1 - wait_packetSize1 / trans_rate1)) * (1 - transiotion_time_ratio1) / 1000
                energy_sum += power_coms1
                trans_flag1 = True
                ...
                if action[4] == 1:
                    power_coms1 = Vars.power_idle_red * (1 - transiotion_time_ratio1)
                elif action[4] == 2:
                    power_coms1 = Vars.power_idle_black * (1 - transiotion_time_ratio1)
                elif action[4] == 3:
                    power_coms1 = Vars.power_idle_green * (1 - transiotion_time_ratio1)
                energy_sum += power_coms1
        elif BSmodel1 == 1:  # BS0处于SM1状态
            power_coms1 = Vars.power_SM1 * (1 - transiotion_time_ratio1)
            energy_sum += power_coms1
        elif BSmodel1 == 2:  # SM2
            power_coms1 = Vars.power_SM2 * (1 - transiotion_time_ratio1)
            energy_sum += power_coms1
        elif BSmodel1 == 3:  # SM3
            power_coms1 = Vars.power_SM2 * (1 - transiotion_time_ratio1)
            energy_sum += power_coms1
    ...
    if transition_time2 != 0:#说明当前处于work model转换阶段
        ...
        if BSmodel2 == 0:
            available_trans2 = trans_rate2 * (1 - transiotion_time_ratio2)
            if wait_packetSize2 >= available_trans2:
                if action[5] == 1: #当前处于red圆
                    #计算energy
                    power_coms2 = Vars.power_trans_full_red * (1-transiotion_time_ratio2)
                elif action[5] == 2: #当前处于black圆
                    power_coms2 = Vars.power_trans_full_black * (1 - transiotion_time_ratio2)
                elif action[5] == 3: #当前处于green圆
                    power_coms2 = Vars.power_trans_full_green * (1 - transiotion_time_ratio2)
                ...
                if action[5] == 1:
                    power_coms2 = (Vars.power_trans_full_red * (wait_packetSize2 / available_trans2) + Vars.power_idle_red * (1 - wait_packetSize2 / trans_rate2)) * (1 - transiotion_time_ratio2) / 1000
                elif action[5] == 2:
                    power_coms2 = (Vars.power_trans_full_black * (wait_packetSize2 / available_trans2) + Vars.power_idle_black * (1 - wait_packetSize2 / trans_rate2)) * (1 - transiotion_time_ratio2) / 1000
                elif action[5] == 3:
                    power_coms2 = (Vars.power_trans_full_green * (wait_packetSize2 / available_trans2) + Vars.power_idle_green * (1 - wait_packetSize2 / trans_rate2)) * (1 - transiotion_time_ratio2) / 1000
                energy_sum += power_coms2
                trans_flag2 = True
                ...
                if action[5] == 1:
                    power_coms2 = Vars.power_idle_red * (1 - transiotion_time_ratio2)
                elif action[5] == 2:
                    power_coms2 = Vars.power_idle_black * (1 - transiotion_time_ratio2)
                elif action[5] == 3:
                    power_coms2 = Vars.power_idle_green * (1 - transiotion_time_ratio2)
                energy_sum += power_coms2
        elif BSmodel2 == 1:  # BS0处于SM1状态
            power_coms2 = Vars.power_SM1 * (1 - transiotion_time_ratio2)
            energy_sum += power_coms2
        elif BSmodel2 == 2:  # SM2
            power_coms2 = Vars.power_SM2 * (1 - transiotion_time_ratio2)
            energy_sum += power_coms2
        elif BSmodel2 == 3:  # SM3
            power_coms2 = Vars.power_SM2 * (1 - transiotion_time_ratio2)
            energy_sum += power_coms2
        transiotion_time_ratio2 = 0
    return transiotion_time_ratio0,transiotion_time_ratio1,transiotion_time_ratio2, energy_sum,arrival_data_matrix0,arrival_data_matrix1,arrival_data_matrix2
#return  transiotion_time_ratio0,transiotion_time_ratio1,transiotion_time_ratio2, energy_sum,arrival_data_matrix0,arrival_data_matrix1,arrival_data_matrix2

#定义基站间模式转换的函数activate-SM1-SM2-SM3
def transition(BSmodel0,BSmodel1,BSmodel2,action,energy_sum,transiotion_time_ratio0,transiotion_time_ratio1,transiotion_time_ratio2,transition_time0,transition_time1,transition_time2,mode_time0,mode_time1,mode_time2):#注意：这里的BSmodel0,BSmodel1,BSmodel2是指的当前时刻的state中的元素
    ...
    if transition_time0 != 0:
        ...
        if BSmodel0 == 0:#Avtivate
            if action[0] == 1: #activate-SM1
                BSmodel0 = 1
                transiotion_time_ratio0 = Vars.delay_Active_SM1 #转换时间为0.0355
                if action[3] == 1:
                    power_coms_trans0 = Vars.power_idle_red * transiotion_time_ratio0#计算转换时的功率
                elif action[3] ==2:
                    power_coms_trans0 = Vars.power_idle_black * transiotion_time_ratio0
                elif action[3] ==3:
                    power_coms_trans0 = Vars.power_idle_green * transiotion_time_ratio0
                ...
                BSmodel0 = 2
                transiotion_time_ratio0 = Vars.delay_Active_SM1 + Vars.delay_SM1_SM2 #0.0355+0.5
                if action[3] == 1:
                ...
                transition_time0 = SM_Hold_Time0-(1-transiotion_time_ratio0)
            ...
            if action[0] == 0: #SM1-Activate
                BSmodel0 = 0
                transiotion_time_ratio0 = Vars.delay_Active_SM1
                power_coms_transition0 = Vars.power_SM1 * transiotion_time_ratio0
                energy_sum += power_coms_transition0
                transition_time0 = 0
                mode_time0 = 0
            elif action[0] == 2: #SM1-SM2
                BSmodel0 = 2
                transiotion_time_ratio0 = Vars.delay_SM1_SM2
                power_coms_transition0 = Vars.power_SM1 * transiotion_time_ratio0
                energy_sum += power_coms_transition0
                SM_Hold_Time0 = Vars.hold_time_SM2
                transition_time0 = SM_Hold_Time0 - (1 - transiotion_time_ratio0)#?
            ...
            if action[0] == 0: #SM2-Activate
                BSmodel0 =0
                transiotion_time_ratio0 = Vars.delay_SM1_SM2 + Vars.hold_time_SM1
                ...
                if transition_time0 > Vars.delay_SM1_SM2:
                    ...
                    transiotion_time_ratio0 = transition_time0 + Vars.hold_time_SM1
                    if transiotion_time_ratio0 > 1:
                        transition_time0 = transiotion_time_ratio0
                else:
                ...
                BSmodel0 = 1
                transiotion_time_ratio0 = Vars.delay_SM1_SM2
                power_coms_transition0 = Vars.power_SM2 * transiotion_time_ratio0
                energy_sum += power_coms_transition0
                if transition_time0 > Vars.delay_SM1_SM2:
                    energy_sum = energy_sum - Vars.power_SM2 * Vars.delay_SM1_SM2
    ...
    if transition_time1 != 0:
        ...
        if BSmodel1 == 0:#Avtivate
            if action[1] == 1: #activate-SM1
                BSmodel1 = 1
                transiotion_time_ratio1 = Vars.delay_Active_SM1 #转换时间为0.0355
                if action[4] == 1:
                    power_coms_trans1 = Vars.power_idle_red * transiotion_time_ratio1#计算转换时的功率
                elif action[4] == 2:
                    power_coms_trans1 = Vars.power_idle_black * transiotion_time_ratio1
                elif action[4] == 3:
                    power_coms_trans1 = Vars.power_idle_green * transiotion_time_ratio1
                ...
                BSmodel1 = 2
                transiotion_time_ratio1 = Vars.delay_Active_SM1 + Vars.delay_SM1_SM2 #0.0355+0.5
                if action[4] == 1:
                ...
                transition_time1 = SM_Hold_Time1 - (1-transiotion_time_ratio1)
                energy_sum += Vars.power_SM2 * Vars.hold_time_SM2 #SM2的hole功率
                mode_time1 = 0
            ...
            if action[1] == 0: #SM1-Activate
                BSmodel1 = 0
                transiotion_time_ratio1 = Vars.delay_Active_SM1
                power_coms_transition1 = Vars.power_SM1 * transiotion_time_ratio1
                energy_sum += power_coms_transition1
                transition_time1 = 0
                mode_time1 = 0
            elif action[1] == 2: #SM1-SM2
                BSmodel1 = 2
                transiotion_time_ratio1 = Vars.delay_SM1_SM2
                power_coms_transition1 = Vars.power_SM1 * transiotion_time_ratio1
                energy_sum += power_coms_transition1
                SM_Hold_Time1 = Vars.hold_time_SM2
                transition_time1 = SM_Hold_Time1- (1 - transiotion_time_ratio1)#?
            ...
            if action[1] == 0: #SM2-Activate
                BSmodel1 =0
                transiotion_time_ratio1 = Vars.delay_SM1_SM2 + Vars.hold_time_SM1
                ...
                if transition_time1 > Vars.delay_SM1_SM2:
                    energy_sum = energy_sum - Vars.power_SM2 * Vars.delay_SM1_SM2
                    transiotion_time_ratio1 = transition_time1 + Vars.hold_time_SM1
                    if transiotion_time_ratio1 > 1:
                        transition_time1 = transiotion_time_ratio1
                else:
                ...
                BSmodel1 = 1
                transiotion_time_ratio1 = Vars.delay_SM1_SM2
                power_coms_transition1 = Vars.power_SM2 * transiotion_time_ratio1
                energy_sum += power_coms_transition1
                if transition_time1 > Vars.delay_SM1_SM2:
                    energy_sum = energy_sum - Vars.power_SM2 * Vars.delay_SM1_SM2
    ...
    if transition_time2 != 0:
        ...
        if BSmodel2 == 0:#Avtivate
            if action[2] == 1: #activate-SM1
                BSmodel2 = 1
                transiotion_time_ratio2 = Vars.delay_Active_SM1 #转换时间为0.0355
                if action[5] == 1:
                    power_coms_trans2 = Vars.power_idle_red * transiotion_time_ratio2#计算转换时的功率
                elif action[5] == 2:
                    power_coms_trans2 = Vars.power_idle_balck * transiotion_time_ratio2
                elif action[5] == 3:
                    power_coms_trans2 = Vars.power_idle_green * transiotion_time_ratio2
                ...
                BSmodel2 = 2
                transiotion_time_ratio2 = Vars.delay_Active_SM1 + Vars.delay_SM1_SM2 #0.0355+0.5
                if action[5] == 1:
                ...
                transition_time2 = SM_Hold_Time2 - (1-transiotion_time_ratio2)
                energy_sum += Vars.power_SM2 * Vars.hold_time_SM2 #SM2的hole功率
                mode_time2 = 0
            ...
            if action[2] == 0: #SM1-Activate
                BSmodel2 = 0
                transiotion_time_ratio2 = Vars.delay_Active_SM1
                power_coms_transition2 = Vars.power_SM1 * transiotion_time_ratio2
                energy_sum += power_coms_transition2
                transition_time2 = 0
                mode_time2 = 0
            elif action[2] == 2: #SM1-SM2
                BSmodel2 = 2
                transiotion_time_ratio2 = Vars.delay_SM1_SM2
                power_coms_transition2 = Vars.power_SM1 * transiotion_time_ratio2
                energy_sum += power_coms_transition2
                SM_Hold_Time2 = Vars.hold_time_SM2
                transition_time2 = SM_Hold_Time2- (1 - transiotion_time_ratio2)#?
            ...
            if action[2] == 0: #SM2-Activate
                BSmodel2 =0
                transiotion_time_ratio2 = Vars.delay_SM1_SM2 + Vars.hold_time_SM1
                ...
                if transition_time2 > Vars.delay_SM1_SM2:
                    energy_sum = energy_sum - Vars.power_SM2 * Vars.delay_SM1_SM2
                    transiotion_time_ratio2 = transition_time2 + Vars.hold_time_SM1
                    if transiotion_time_ratio2 > 1:
                        transition_time2 = transiotion_time_ratio2
                else:
                ...
                BSmodel1 = 2
                transiotion_time_ratio2 = Vars.delay_SM1_SM2
                power_coms_transition2 = Vars.power_SM2 * transiotion_time_ratio2
                energy_sum += power_coms_transition2
                if transition_time2 > Vars.delay_SM1_SM2:
                    energy_sum = energy_sum - Vars.power_SM2 * Vars.delay_SM1_SM2
            ...
            if action[2] == 0:  # SM3-active
                ...
                transiotion_time_ratio2 = 0
                transition_time2 = 0
    return BSmodel0,BSmodel1,BSmodel2,energy_sum,transiotion_time_ratio0,transiotion_time_ratio1,transiotion_time_ratio2,transition_time0,transition_time1,transition_time2,mode_time0,mode_time1,mode_time2
#return BSmodel0,BSmodel1,BSmodel2,energy_sum,transiotion_time_ratio0,transiotion_time_ratio1,transiotion_time_ratio2,transition_time0,transition_time1,transition_time2,mode_time0,mode_time1,mode_time2
